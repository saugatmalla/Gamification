<template>
    <div class="grid grid-cols-1 md:grid-cols-2 place-items-stretch gap-3">
        ...
        <div class="col-span-1 min-w-full min-h-[200px] max-h-[200px] relative">
            <label
                class="block text-sm font-medium text-txt-primary dark:text-gray-400 text-center"
            >Preview</label>
            <img v-if="imagePreviewUrl" :src="imagePreviewUrl" class="w-full h-full object-contain" />
            <div
...
if (props.defaultImageUrl && props.defaultImageUrl !== '') {
    imagePreviewUrl.value = props.defaultImageUrl;
}
...
const handleFileUpload = async (event: any) => {
    ...
    if (filesArray.length > 0) {
        imagePreviewUrl.value = URL.createObjectURL(filesArray[0]);

        files.value.push(filesArray[0]);
    }
}
